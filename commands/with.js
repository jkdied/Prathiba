  ...
  async execute(msg, args){
    ...
    if (deptime !== null && timeout - (Date.now() - deptime) > 0) {
      let time = ms(timeout - (Date.now() - deptime));
        ...
        .setColor('RANDOM')
        .setDescription(`Wait for **${time}**. It takes time to transfer stuff from your brain, silly !`)
      ...
      if(args[2] == null){
        ...
        .setDescription(`${user.username}, Do you really think you can extract nothing ?\n Mention how much marks you want to transfer, for god\'s sake!`)
        msg.channel.send(nullval);
      }
      else if(args[2] == 'all'){
        const depforall = userbankfetch;
        db.set(`deptime_${msg.author.id}`, Date.now());
        db.add(usermoney, depforall)
        db.subtract(userbank, depforall)
        const depositedall = new Discord.MessageEmbed()
        .setAuthor(`${user.username}`)
        .setDescription(`Extracted ${depforall} marks from your brain.`)
        msg.channel.send(depositedall)
        }
      else if(!isNaN(args[2])){
        const amount = Math.floor(args[2]);
        if(amount > userbankfetch){
          const notenuf = new Discord.MessageEmbed()
          .setAuthor(`${user.username}`)
          .setDescription(`You do not have that much marks stored in your brain, Idiot.`)
          msg.channel.send(notenuf)
        }
        else if(amount < userbankfetch || amount == userbankfetch){
          if(amount < 0){
            const negwith = new Discord.MessageEmbed()
            .setAuthor(`${user.username}`)
            .setDescription(`You cannot extract Negative marks ! \nAre you crazy ?`)
            msg.channel.send(negwith);
          }
          else{
            db.set(`deptime_${msg.author.id}`, Date.now());
            db.add(usermoney, amount)
            db.subtract(userbank, amount)
            const depositamount = new Discord.MessageEmbed()
            .setAuthor(`${user.username}`)
            .setDescription(`Extracted ${amount} marks from your brain.`)
        ...
        .setAuthor(`${user.username}`)
        .setDescription(`Do you expect to extract random crap ?\nYou will have to define an amount of how much marks you want to extract.`)
